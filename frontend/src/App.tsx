...
const { projects, activeProject, analytics, budget, video, bgImage, loadWorkspace, createProject } = useRoadRead();  const [stage, setStage] = useState<AppStage>("auth");
  ...
  const handleCreate = async (name: string, fileName: string, location: RoadLocation | null, file: File | null) => { setStage("processing"); try { await createProject(name, fileName, location, file); setStage("app"); } catch (error) { console.error("RoadRead analysis failed", error); setStage("onboarding"); } };
  ...
  if (stage === "processing") return <ProcessingView />;
